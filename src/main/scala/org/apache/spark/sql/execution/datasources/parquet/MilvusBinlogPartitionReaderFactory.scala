 ...
 */
case class MilvusBinlogPartitionReaderFactory(
                                           sqlConf: SQLConf,
  ...
  private def getFooter(file: PartitionedFile): ParquetMetadata = {
    ...
    if (aggregation.isEmpty) {
      MParquetFooterReader.readFooter(conf, filePath, SKIP_ROW_GROUPS)
    } else {
      // For aggregate push down, we will get max/min/count from footer statistics.
      MParquetFooterReader.readFooter(conf, filePath, NO_FILTER)
    }
  }
  ...
  private def createVectorizedReader(file: PartitionedFile): MVectorizedParquetRecordReader = {
    val vectorizedReader = buildReaderBase(file, createParquetVectorizedReader)
      .asInstanceOf[MVectorizedParquetRecordReader]
  ...
  private def createParquetVectorizedReader(
                                             ...
                                             convertTz: Option[ZoneId],
                                             datetimeRebaseSpec: RebaseSpec,
                                             int96RebaseSpec: RebaseSpec): MVectorizedParquetRecordReader = {
    val taskContext = Option(TaskContext.get())
    val vectorizedReader = new MVectorizedParquetRecordReader(
      convertTz.orNull,
