...
import org.apache.spark.sql.execution.datasources.parquet.MilvusBinlogFileFormat.{logInfo, logWarning}
import org.apache.spark.sql.execution.datasources.parquet.{ParquetFilters, MParquetFooterReader, ParquetLogRedirector, ParquetOptions, ParquetOutputWriter, ParquetReadSupport, MilvusToSparkSchemaConverter, MilvusBinlogUtils, ParquetWriteSupport, VectorizedParquetRecordReader}
...
class MilvusBinlogFileFormat
  ...
  override def equals(other: Any): Boolean = other.isInstanceOf[MilvusBinlogFileFormat]
  ...
  override def inferSchema(
    ...
    MilvusBinlogUtils.inferSchema(sparkSession, parameters, files)
  }
  ...
  override def supportBatch(sparkSession: SparkSession, schema: StructType): Boolean = {
    ...
    MilvusBinlogUtils.isBatchReadSupportedForSchema(conf, schema)
  }
  ...
  override def buildReaderWithPartitionValues(
    ...
    val enableVectorizedReader: Boolean =
      MilvusBinlogUtils.isBatchReadSupportedForSchema(sqlConf, resultSchema)
    ...
    (file: PartitionedFile) => {
      ...
      lazy val footerFileMetaData =
        MParquetFooterReader.readFooter(sharedConf, filePath, SKIP_ROW_GROUPS).getFileMetaData
...
object MilvusBinlogFileFormat extends Logging {
  private[parquet] def readSchema(
                                   footers: Seq[Footer], sparkSession: SparkSession): Option[StructType] = {

    val converter = new MilvusToSparkSchemaConverter(
      sparkSession.sessionState.conf.isParquetBinaryAsString,
      sparkSession.sessionState.conf.isParquetINT96AsTimestamp,
    ...
    ThreadUtils.parmap(partFiles, "readingParquetFooters", 8) { currentFile =>
      try {
        ...
        Some(new Footer(currentFile.getPath(),
          MParquetFooterReader.readFooter(
  ...
  def mergeSchemasInParallel(
    ...
    val reader = (files: Seq[FileStatus], conf: Configuration, ignoreCorruptFiles: Boolean) => {
      ...
      readParquetFootersInParallel(conf, files, ignoreCorruptFiles)
        .map(MilvusBinlogFileFormat.readSchemaFromFooter(_, converter))
    }
  ...
  def readSchemaFromFooter(
                            footer: Footer, converter: MilvusToSparkSchemaConverter): StructType = {
