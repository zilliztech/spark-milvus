...
class MilvusToSparkSchemaConverter(
  ...
  private def convertPrimitiveField(
    ...
    def makeDecimalType(maxPrecision: Int = -1): DecimalType = {
      ...
      MParquetSchemaConverter.checkConversionRequirement(
...
class MSparkToParquetSchemaConverter(
  ...
  def convert(catalystSchema: StructType): MessageType = {
    Types
      ...
      .named(MParquetSchemaConverter.SPARK_PARQUET_SCHEMA_NAME)
  }
  ...
  def convertField(field: StructField): Type = {
    ...
    if (useFieldId && MilvusBinlogUtils.hasFieldId(field)) {
      converted.withId(MilvusBinlogUtils.getFieldId(field))
    } else {
      converted
    }
  }
...
object MParquetSchemaConverter {
  val SPARK_PARQUET_SCHEMA_NAME = "spark_schema"

  val EMPTY_MESSAGE: MessageType =
    Types.buildMessage().named(MParquetSchemaConverter.SPARK_PARQUET_SCHEMA_NAME)
