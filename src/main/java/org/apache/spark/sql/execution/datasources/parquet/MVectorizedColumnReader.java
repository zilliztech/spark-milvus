 ...
 */
public class MVectorizedColumnReader {
  /**
   ...
   */
  private MVectorizedRleValuesReader defColumn;

  /**
   * Vectorized RLE decoder for repetition levels
   */
  private MVectorizedRleValuesReader repColumn;

  /**
   * Factory to get type-specific vector updater.
   */
  private final MParquetVectorUpdaterFactory updaterFactory;

  /**
   * Helper struct to track intermediate states while reading Parquet pages in the column chunk.
   */
  private final MParquetReadState readState;
  ...
  public MVectorizedColumnReader(
  ...
  void readBatch(
    ...
    MParquetVectorUpdater updater = updaterFactory.getUpdater(descriptor, column.dataType());
      ...
      if (isCurrentPageDictionaryEncoded) {
        ...
        if (readState.maxRepetitionLevel == 0) {
          defColumn.readIntegers(readState, dictionaryIds, column, definitionLevels,
            (MVectorizedValuesReader) dataColumn);
        } else {
          repColumn.readIntegersRepeated(readState, repetitionLevels, defColumn, definitionLevels,
            dictionaryIds, column, (MVectorizedValuesReader) dataColumn);
        }
        ...
        if (column.hasDictionary() || (startRowId == pageFirstRowIndex &&
          ...
          column.setDictionary(new MParquetDictionary(dictionary, needTransform));
        } else {
        ...
        MVectorizedValuesReader valuesReader = (MVectorizedValuesReader) dataColumn;
        if (readState.maxRepetitionLevel == 0) {
  ...
  private void initDataReader(
    ...
    if (dataEncoding.usesDictionary()) {
      ...
      this.dataColumn = new MVectorizedRleValuesReader();
      this.isCurrentPageDictionaryEncoded = true;
    } else {
  ...
  private ValuesReader getValuesReader(Encoding encoding) {
    switch (encoding) {
      case PLAIN:
        return new MVectorizedPlainValuesReader();
      case DELTA_BYTE_ARRAY:
        return new MVectorizedDeltaByteArrayReader();
      case DELTA_BINARY_PACKED:
        return new MVectorizedDeltaBinaryPackedReader();
      case RLE:
        PrimitiveType.PrimitiveTypeName typeName =
        ...
        if (typeName == BOOLEAN) {
          return new MVectorizedRleValuesReader(1);
        } else {
  ...
  private int readPageV1(DataPageV1 page) throws IOException {
    ...
    this.repColumn = new MVectorizedRleValuesReader(rlBitWidth);

    int dlBitWidth = BytesUtils.getWidthFromMaxInt(descriptor.getMaxDefinitionLevel());
    this.defColumn = new MVectorizedRleValuesReader(dlBitWidth);
  ...
  private int readPageV2(DataPageV2 page) throws IOException {
    ...
    int rlBitWidth = BytesUtils.getWidthFromMaxInt(descriptor.getMaxRepetitionLevel());
    repColumn = new MVectorizedRleValuesReader(rlBitWidth, false);
    repColumn.initFromPage(pageValueCount, page.getRepetitionLevels().toInputStream());

    int dlBitWidth = BytesUtils.getWidthFromMaxInt(descriptor.getMaxDefinitionLevel());
    defColumn = new MVectorizedRleValuesReader(dlBitWidth, false);
