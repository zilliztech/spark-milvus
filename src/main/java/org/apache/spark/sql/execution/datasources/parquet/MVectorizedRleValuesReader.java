 ...
 */
public final class MVectorizedRleValuesReader extends ValuesReader
    implements MVectorizedValuesReader {
  ...
  public MVectorizedRleValuesReader() {
    this.fixedWidth = false;
    this.readLength = false;
  }

  public MVectorizedRleValuesReader(int bitWidth) {
    this.fixedWidth = true;
    this.readLength = bitWidth != 0;
    init(bitWidth);
  }

  public MVectorizedRleValuesReader(int bitWidth, boolean readLength) {
    this.fixedWidth = true;
  ...
  public void readBatch(
      MParquetReadState state,
      WritableColumnVector values,
      WritableColumnVector defLevels,
      MVectorizedValuesReader valueReader,
      MParquetVectorUpdater updater) {
    if (defLevels == null) {
  ...
  public void readIntegers(
    ...
    if (defLevels == null) {
      readBatchInternal(state, values, nulls, valueReader,
        new MParquetVectorUpdaterFactory.IntegerUpdater());
    } else {
  ...
  public void readBatchRepeated(
      ...
      MVectorizedRleValuesReader defLevelsReader,
      WritableColumnVector defLevels,
      WritableColumnVector values,
      MVectorizedValuesReader valueReader,
      MParquetVectorUpdater updater) {
  ...
  public void readIntegersRepeated(
    ...
    readBatchRepeatedInternal(state, repLevels, defLevelsReader, defLevels, values, nulls, false,
      valueReader, new MParquetVectorUpdaterFactory.IntegerUpdater());
  }
  ...
  private static class DefLevelProcessor {
    private final MVectorizedRleValuesReader reader;
    private final MParquetReadState state;
    ...
    private final MVectorizedValuesReader valueReader;
    private final MParquetVectorUpdater updater;

    DefLevelProcessor(
        MVectorizedRleValuesReader reader,
        MParquetReadState state,
        WritableColumnVector defLevels,
        WritableColumnVector values,
        WritableColumnVector nulls,
