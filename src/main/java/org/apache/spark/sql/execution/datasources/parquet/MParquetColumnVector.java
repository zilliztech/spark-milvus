 ...
 */
final class MParquetColumnVector {
  private final ParquetColumn column;
  private final List<MParquetColumnVector> children;
  ...
  private MVectorizedColumnReader columnReader;

  MParquetColumnVector(
  ...
  MParquetColumnVector(
    ...
    if (isPrimitive) {
      ...
      for (int i = 0; i < column.children().size(); i++) {
        MParquetColumnVector childCv = new MParquetColumnVector(column.children().apply(i),
   ...
   */
  List<MParquetColumnVector> getChildren() {
    return children;
  }
   ...
   */
  List<MParquetColumnVector> getLeaves() {
    List<MParquetColumnVector> result = new ArrayList<>();
    getLeavesHelper(this, result);
    return result;
  }

  private static void getLeavesHelper(MParquetColumnVector vector, List<MParquetColumnVector> coll) {
    if (vector.isPrimitive) {
      coll.add(vector);
    } else {
      for (MParquetColumnVector child : vector.children) {
  ...
  void assemble() {
    ...
    if (type instanceof ArrayType || type instanceof MapType) {
      for (MParquetColumnVector child : children) {
        child.assemble();
      }
      assembleCollection();
   ...
   */
  MVectorizedColumnReader getColumnReader() {
   ...
   * column.
   */
  void setColumnReader(MVectorizedColumnReader reader) {
    if (!isPrimitive) {
