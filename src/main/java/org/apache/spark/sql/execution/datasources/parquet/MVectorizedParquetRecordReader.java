 ...
 */
public class MVectorizedParquetRecordReader extends MSpecificParquetRecordReaderBase<Object> {
   ...
   */
  private MParquetColumnVector[] columnVectors;
  ...
  public MVectorizedParquetRecordReader(
      ZoneId convertTz,
  ...
  // For test only.
  public MVectorizedParquetRecordReader(boolean useOffHeap, int capacity) {
    this(
      null,
      "CORRECTED",
      "UTC",
  ...
  private void initBatch(
    ...
    columnVectors = new MParquetColumnVector[sparkSchema.fields().length];
    for (int i = 0; i < columnVectors.length; i++) {
      columnVectors[i] = new MParquetColumnVector(parquetColumn.children().apply(i),
  ...
  public boolean nextBatch() throws IOException {
    for (MParquetColumnVector vector : columnVectors) {
      vector.reset();
    }
    ...
    for (MParquetColumnVector cv : columnVectors) {
      for (MParquetColumnVector leafCv : cv.getLeaves()) {
        MVectorizedColumnReader columnReader = leafCv.getColumnReader();
  ...
  private void initColumnReader(PageReadStore pages, MParquetColumnVector cv) throws IOException {
    if (!missingColumns.contains(cv.getColumn())) {
      if (cv.getColumn().isPrimitive()) {
        ParquetColumn column = cv.getColumn();
        MVectorizedColumnReader reader = new MVectorizedColumnReader(
        ...
        for (MParquetColumnVector childCv : cv.getChildren()) {
