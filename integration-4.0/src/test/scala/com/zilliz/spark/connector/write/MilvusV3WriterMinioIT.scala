...
/** Integration tests for [[MilvusV3Writer]].
  *
  * Prerequisites: MinIO running at localhost:9000 with a pre-created `a-bucket`
  * and minioadmin/minioadmin credentials.
  */
class MilvusV3WriterMinioIT extends AnyFunSuite with Matchers {

  test("Write DataFrame using the V3 writer to Minio") {
    val spark = SparkSession
      .builder()
      .appName("V3WriterMinioTest")
      .master("local[*]")
      .getOrCreate()
