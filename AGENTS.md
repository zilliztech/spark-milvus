...
procedure); the two older brute-force search paths are deleted. The search
entry is moving out of the static `MilvusSearch.search` into the DataFrame plan:
a pushed-down TopN over a distance function for one query, a `knn` join for a
query set (decision 16 as revised on 2026-09-20; not yet coded). `stats` writes
