...
class SnapshotCatalogTest extends AnyFunSuite {
  ...
  test("reads a V3 snapshot: segment id, partition from the path, version pinned") {
    withDir { dir =>
      ...
      assert(snapshot.origin == SnapshotOrigin.Catalog("files/snapshots/10/metadata/1.json"))
      val seg = snapshot.segments.head
      assert(seg.id == 30L)
      assert(seg.partitionId == 20L)
      assert(seg.storageVersion == 3)
      assert(seg.rows.isEmpty)
      assert(seg.layout == SegmentLayout.Manifest("files/insert_log/10/20/30", 7L))
      assert(seg.deletes == DeleteFiles.InManifest)
      assert(seg.hasData)
    }
  }

  test("a V3 path outside the insert_log layout falls back to the snapshot's first partition") {
    withDir { dir =>
      write(
        dir,
      ...
      val seg = catalog(dir).read("files/snapshots/10/metadata/1.json").segments.head
      assert(seg.partitionId == 20L)
    }
  }
  ...
  test("latest, by name and as-of pick among the collection's snapshots") {
    withDir { dir =>
      ...
      val missing = intercept[IllegalArgumentException](c.byName("files", 10L, "nope"))
  ...
  test("metadataPrefix follows DataCoord's layout") {
    assert(SnapshotCatalog.metadataPrefix("files", 10L) == "files/snapshots/10/metadata/")
    assert(SnapshotCatalog.metadataPrefix("files/", 10L) == "files/snapshots/10/metadata/")
  ...
  test("a snapshot in another bucket than the catalog's is refused") {
    withDir { dir =>
      write(dir, "files/snapshots/10/metadata/1.json", snapshotJson("s1", 100L))
      val c = new SnapshotCatalog(new LocalObjectStore(dir.toString), "a", V2SegmentResolver.Unavailable)
      val err = intercept[IllegalArgumentException](c.read("s3a://b/files/snapshots/10/metadata/1.json"))
  ...
  test("a snapshot over the size limit is refused before parsing") {
    withDir { dir =>
      write(dir, "files/snapshots/10/metadata/1.json", snapshotJson("s1", 100L))
      val c = new SnapshotCatalog(new LocalObjectStore(dir.toString), "", V2SegmentResolver.Unavailable, maxJsonBytes = 10L)
      val err = intercept[IllegalArgumentException](c.read("files/snapshots/10/metadata/1.json"))
      assert(err.getMessage.contains("byte limit"))
    }
  }

  test("fromMetadata rejects a snapshot missing its parts") {
    def bad(m: SnapshotMetadata) =
      SnapshotCatalog
        .fromMetadata(m, SnapshotOrigin.Options, new LocalObjectStore(""), "", V2SegmentResolver.Unavailable)
        .left
        .get
        .getMessage
    assert(bad(SnapshotMetadata(snapshotInfo = null, collection = Collection(CollectionSchema("c", fields = Seq.empty)))).contains("snapshot_info"))
    assert(bad(SnapshotMetadata(snapshotInfo = SnapshotInfo("s"), collection = null)).contains("collection"))
    assert(bad(SnapshotMetadata(snapshotInfo = SnapshotInfo("s"), collection = Collection(null))).contains("collection.schema"))
    val empty = bad(
      SnapshotMetadata(
  ...
  test("V2 segments are materialized through the resolver and deduplicated by slot") {
    withDir { dir =>
      write(
        dir,
        "files/snapshots/10/metadata/1.json",
        snapshotJson("s1", 100L, Seq.empty).replace("\"manifest_list\": []", "\"manifest_list\": [\"files/snapshots/10/manifests/1/40.avro\"]")
      )
      val resolver = new V2SegmentResolver {
        def resolve(paths: Seq[String], bucket: String, store: com.zilliz.milvus.storage.io.ObjectStore, v: Int) = {
          ...
          Right(
            Seq(
              V2SegmentInfo(
                ...
                storageVersion = 2L,
                columnGroups = Seq(
                  V2ColumnGroup(Seq(100L, 0L, 1L), Seq("a.parquet"), Seq(2L), slotFieldId = 3L),
                  V2ColumnGroup(Seq(100L), Seq("b.parquet"), Seq(2L), slotFieldId = 100L)
                ),
                deltaLogs = Seq(V2DeltaLogFile(1L, "d.log", 1L))
              ),
              V2SegmentInfo(41L, 20L, 0L, 2L, Seq.empty, Seq(V2DeltaLogFile(2L, "l0.log", 3L)))
            )
          )
        }
      }
      val snapshot = new SnapshotCatalog(new LocalObjectStore(dir.toString), "", resolver)
        .read("files/snapshots/10/metadata/1.json")
      val data = snapshot.dataSegments
      assert(data.map(_.id) == Seq(40L))
      val groups = data.head.layout.asInstanceOf[SegmentLayout.ColumnGroups].groups
      assert(groups.find(_.slotFieldId == 3L).get.fieldIds == Seq(0L, 1L))
      assert(data.head.rows.contains(2L))
      assert(data.head.deletes == DeleteFiles.Listed(Seq(V2DeltaLogFile(1L, "d.log", 1L))))
