...
import io.milvus.storage.MilvusStorageColumnGroups
...
final class WrittenColumnGroups private[exec] (handle: Long)
  ...
  override def close(): Unit = synchronized {
    if (!released) {
      released = true
      MilvusStorageColumnGroups.destroy(handle)
    }
  }
}
