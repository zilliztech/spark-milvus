...
| `milvus.read.batch.max.bytes` | Long | No | 33554432 | Positive target byte limit for one native record batch; delivered as `reader.record_batch_max_size`. The current upstream maximum is 4294967296 (4 GiB). In an exact search it is also the base block one engine call takes: 32 MiB when unset; the batched distance entry tiles the block internally, so the block only sets the fixed cost per call and the task's memory, and 32 MiB and 128 MiB measured the same per-core rate. The choice is logged as `exact scan batch: …` (design: [search-resources.html](design/architecture/search-resources.html#block)). |
...
The runtime extracts and verifies them once per class loader. An incompatible
explicit `knowhere.native.path` is rejected. See [building and validation](contributing.md#unified-native-bundle).
