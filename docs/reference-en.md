...
| `milvus.read.batch.max.bytes` | Long | No | 33554432 for reads; automatic for exact search | Positive target byte limit for one native record batch; delivered as `reader.record_batch_max_size`. The current upstream maximum is 4294967296 (4 GiB). In an exact search it is also the base block one engine call scans: when unset, each executor derives it from its machine's L3 × CPU quota × 0.6 ÷ task slots, clamped to 1–32 MiB and rounded down to a power of two, 4 MiB when L3 cannot be read; on a cluster the CPU quota is at most the executor's cores over the host's, so a pod without a CPU limit does not count the whole host as its own; the choice is logged as `exact scan batch: …`. A set value is used as it is (design: [search-resources.html](design/architecture/search-resources.html#block)). |
