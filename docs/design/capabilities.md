...
| V7 | 持久化索引向量查询 | `MilvusSearch.search(..., mode = index)`，输入查询集 | spark.read、core.index | V2、R7、SegmentReader.take；与 V5 共用段内执行器接口、合并与回表；快照校验与规划已在 UAT 验证，加载云上索引要含 Cardinal 的原生构建（[2.9](architecture/vector-search.html#interop)）；[查询契约](architecture/vector-search.html#api) | P2 |
