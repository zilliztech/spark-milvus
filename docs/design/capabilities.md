...
| 编号 | 功能 | 用户入口 | 实现位置 | 依赖或前提 | 优先级 |
|---|---|---|---|---|---|
| A1 | 快照 | `create_snapshot('db.coll', 'name')`、`drop_snapshot`、`list_snapshots`、`describe_snapshot`；`restore_snapshot` 恢复对象存储上的快照（W8） | spark.extensions → spark.procedure → client.api | 五个都已实现；连接器转发在线侧快照 RPC，不自行决定建前 Flush；`restore_snapshot` 走 `RestoreExternalSnapshot`，调用前按 W8 的根规则核对快照 | P1 |
