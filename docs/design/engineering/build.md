...
├── platforms.py                平台适配器：Elf 与 MachO 两个类，库名、依赖表读取、查找路径改写、系统库集合与 `ldconfig -p` 的架构标签、工具链；加平台即加一个类
