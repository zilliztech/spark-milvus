...
> **2026-09-23 状态。** 当时的 gitlink 为 milvus-storage `7eb13578`、Knowhere `29210a33`；2026-09-23 起为 milvus-storage `d287d05`、Knowhere `b9497f4f`，linux-x86_64 统一包按此在本机重建，真实 UAT 查询按该包复测，结果见 [search-resources.html 3.6](../architecture/search-resources.html#range-read)。2026-09-25 起 Knowhere gitlink 为 `523518f7`（反序列化按引用共享调用方的 BinarySet，加载峰值少一份拷贝），统一包待按此重建。四个平台都有源码构建 profile 和验收记录。linux-x86_64：本机已完成三次统一构建，两道门都通过，`make test` 为 1,160 项成功、0 项失败、149 项取消。darwin-aarch64：本机从空 Conan 缓存完整构建一次，没有改动任何 recipe，Knowhere C API 测试 2/2，221 个动态库与 203 个别名入包，逐库 Mach-O 诊断 221/221 通过，两个全新 JVM 的双向 `System.load` 均退出 0，平台 JAR 148,924,666 字节、SHA-256 `97c2b5f7`…，`make package` 与显式 `NATIVE_BUNDLE` 两条路径的 `verifyNativeBundle` 都通过；DiskANN 按平台关闭并写进 `with_diskann=false`。该平台只有开源变体：Knowhere 钉的 Cardinal `v2.5.112` 与 `v3.0.8` 在 darwin-aarch64 上编不过（Darwin 的 `uint64_t`/`size_t` 类型不同、Linux 专有的 `MADV_HUGEPAGE` 与 `O_DIRECT`、直接 `#include <cblas.h>`、SVE intrinsic），Knowhere 自己的 macOS CI 也不编 Cardinal，Cardinal 的验证留在 Linux。darwin-aarch64 的 `make test`（带该包）为 1,167 项成功、0 项失败、152 项取消，取消的全部是缺少 `MILVUS_UAT_*`/`MILVUS_JNI_S3_BUCKET` 环境的 UAT 用例，原生用例（StorageNativeTest、WriterRoundTripTest、KnowhereBuffersTest、NativeVectorSearchTest）实际运行。darwin-x86_64（2026-09-23）：在一台 macOS 15.0 的 Intel 机器（i7-14700KF，28 线程）上构建，Conan 图 70 个包全部从源码编译、没有改动任何 recipe，`--jobs 12` 从锁定依赖到入包 65 分钟；Knowhere C API 测试 2/2；221 个动态库与 203 个别名入包，逐库 Mach-O 诊断无失败；两个全新 JVM 的双向 `System.load` 均退出 0；平台 JAR 170,540,032 字节、SHA-256 `451f7ea7`…；`make package` 与显式 `NATIVE_BUNDLE` 两条路径的 `verifyNativeBundle` 都通过；`make test`（带该包）为 1,251 项成功、0 项失败、152 项取消，取消的全是缺 `MILVUS_UAT_*`/`MILVUS_JNI_S3_BUCKET` 环境的 UAT 用例，原生用例实际运行。编译器是 Apple clang 17.0.0：Knowhere 自 d48fda72（#1748）起用 `std::atomic_ref`，libc++ 19 才提供它，Apple clang 16 编不过；macOS 15.0 装不了 Command Line Tools 16.3 及以后的版本，这台机器用 CLT 16.4 安装包解开后的工具链（clang 二进制要求 macOS 14.0 以上），SDK 路径写在这台机器的 Conan `global.conf` 里。`profiles/darwin-x86_64` 比 darwin-aarch64 多一行：folly 的 CMake 在 x86_64 上给 `crypto/detail/MathOperation_Simple.cpp` 加 `-mno-sse2`，macOS 15 SDK 的 `math.h` 声明了 `_Float16` 函数，clang 在 SSE2 关闭时不接受这个类型，所以 folly 单独用 macOS 14.5 SDK 编，folly 不再对该文件关闭 SSE2 后删掉这一行。同一次构建修了 `build.py`：`conanrun.sh` 导出的 `DYLD_LIBRARY_PATH` 让 CMake 启动的 cargo 加载 Conan 的 `libiconv.2.dylib` 而中止，现在 Mach-O 上只保留 fallback 变量。linux-aarch64（2026-09-21）：在 UAT `aws-milvus` 集群的一个 Graviton4 pod（i8g.4xlarge，Neoverse-V2，16 vCPU，有 SVE2）里按 Dockerfile 的 toolchain 阶段装好工具链后从空 Conan 缓存构建：Conan 图 68 个包全部从源码编译、没有改动任何 recipe，`--jobs 14` 用时 40 分钟；带 Cardinal 的正式构建复用该缓存与 `conan.lock`，从快照到入包 18.6 分钟；Knowhere C API 测试 3/3（含 DiskANN 验收，65.9 秒）；164 个动态库与 173 个别名入包（linux-x86_64 的 162 个加 Cardinal 两个，数量相同）；两个全新 JVM 的双向 `System.load` 均退出 0；逐库 `ldd -r` 诊断有 4 个库未通过（folly、opentelemetry_proto_grpc、两个 upb），是 linux-x86_64 那 5 个的子集；平台 JAR 261,989,676 字节、SHA-256 `222fe618`…；`verifyNativeBundle` 与 `knowhereSmoke` 通过；`sbt test`（带该包）1,206 项成功、0 项失败、153 项取消，取消的全是缺 `MILVUS_UAT_*` 环境的 UAT 用例，原生用例实际运行。为此补的四处：`profiles/linux-aarch64`（与 x86_64 只差 `arch`）；`Cardinal.cmake` 与 `cardinal/Sources.cmake` 按架构选源文件与 flag，此前只有 x86_64 写法，`-mprefer-vector-width=512` 在 aarch64 的 GCC 上是未知选项；`stage.py` 用 `ldconfig -p` 找系统 libaio 与 libz 时写死的 `x86-64` 标签移进 ELF 适配器按机器选（aarch64 的标签是 `AArch64`）；ELF 适配器的 `library_glob` 在 f077561 里丢了族通配，`libknowhere.so*` 匹配不到 `libknowhere_c.so.1` 与 `libknowhere_jni.so`，staging 在 Linux 上必然失败——上面 linux-x86_64 的三次构建用的是适配层重构之前的 `stage.py`；2026-09-23 按 `d287d05` / `b9497f4f` 的重建记录在提交 a8ab261（2026-09-23 12:02 +0800，root 套件 1,245 项成功、0 项失败、152 项取消），晚于修复 `library_glob` 的 3e316e5（2026-09-22），其工作目录与 provenance 不在写这份文档的机器上，它是否用了修复后的适配层代码待确认。该包按 Cardinal 的 `-march=native` 要求 SVE2 主机；Knowhere 自身在 linux-aarch64 上只编 NEON（`KNOWHERE_USE_SVE` 未定义，与 darwin 分支相同），SVE 是待办。尚未做：darwin-aarch64 的真实 UAT 查询，空 Conan 缓存的 Docker 重建，确认 linux-x86_64 的 2026-09-23 重建用的是当前适配层代码，linux-aarch64 的真实数据验证已做：UAT 实例当时已停机，aarch64 包直接读 S3 上 `spark_perf_1m_1024`（100 万行 × 1024 维）的快照，exact 与 Milvus 建的 HNSW 两种模式各跑 1/10/100 条查询，HNSW 相对 exact 的召回 0.8000、0.8400、0.8390，与本机 x86_64 在同一快照上的结果逐项相同；记录在工作区报告 `docs/reports/spark-milvus-linux-aarch64-native-build-2026-09-21/`。

## 1 依赖库有哪些，各自做什么

**每个平台的原生 JAR 分三层：两个 JNI 入口，两个引擎（milvus-storage 一个库，Knowhere 两个），其余是两个引擎共用的 Conan 依赖（linux-x86_64 上 157 个、合计 162 个动态库；darwin-aarch64 与 darwin-x86_64 上各 216 个、合计 221 个）。** 依赖由 `native-build/dependencies.json` 固定的 32 个直接 Conan 包和它们的传递依赖组成，完整清单以该文件和包内 `manifest.properties` 为准，本页不展开。库名在 Linux 是 `libfoo.so.1`，在 macOS 是 `libfoo.1.dylib`；JVM 侧不写后缀，入口库名由清单声明，其余由 `System.mapLibraryName` 推出。
...
| 库 | 做什么 | 源码来自 |
|---|---|---|
| `milvus-storage` | Milvus 段文件格式（V2 packed、V3 loon）的读写：manifest、列组 Parquet、delete 文件、统计，以及对象存储文件系统。Rust 桥 prsbridge（Lance、Vortex 格式）静态编进它 | gitlink 子模块 `milvus-storage`，`d287d05`：上游 main `1ff1682` 加决策 31 的一个提交，过渡期取自 fork `Thor-ChenBiao/milvus-storage` 的 `parquet-eager-prebuffer`（设计索引决策日志 2026-09-23） |
...
| Ninja | 随发行版 | Homebrew |
| C/C++ 编译器 | GCC、G++ 12 | Apple Clang 17 以上（Knowhere 用 `std::atomic_ref`），另加 Homebrew `libomp` 提供 OpenMP |
...
|---|---|---|---|---|
| A. Docker 构建 | `docker build --build-arg PUBLISH_MAVEN=false -t spark-milvus .` | Docker 与 BuildKit；网络能到 JFrog、GitHub、crates.io。工具链由 Dockerfile 安装，本机无需准备 | 统一包 + assembly。Conan、Cargo、ccache、Coursier、Ivy、sbt 缓存各在一个 `sharing=locked` 的 cache mount 里，失败的 RUN 不丢已完成的依赖。worker 是哪个架构就出哪个架构的包；只有 Linux | Jenkins 发布流水线；验证"空缓存能否完整构建" |
| B. 本机构建 | `make native-bundle NATIVE_JOBS=50`，然后 `make package` | 第 2 节本平台一列；PATH 上先出现上表的版本 | 同 A，产物在 `target/native-build/<平台>/`；`--conan-lock` 复用审核过的 lock，`--no-remote` 只用缓存。输入变化要换新工作目录，缓存仍复用。产物依赖构建机的 glibc：在比 Spark 镜像（Ubuntu 22.04，glibc 2.35）更新的系统上编出的包在该镜像里加载失败（2026-09-20 实测：本机包要求 `GLIBC_2.38`，sbt 校验拒绝），要给 Spark 镜像用的包应在[开发容器](devcontainer.html)或方案 A 里编 | 改 storage、Knowhere、依赖版本或 CMake 规则的开发；工具链也可由[开发容器](devcontainer.html)提供（设计稿） |
...
│   ├── Cardinal.cmake          两代 Cardinal 插件的目标，按 CMAKE_SYSTEM_PROCESSOR 选源码清单并加逐文件 flag
│   ├── cardinal/Sources.cmake  Cardinal 的源码清单：公共部分加 x86_64、aarch64 各一份
...
├── profiles/
│   └── <平台>                  Conan profile，每平台一份；今天四个平台都有
├── dependencies.json           32 个直接依赖的 recipe revision、版本冲突策略、排除项
├── conanfile.py                消费者：把 references 以 force=True 放进一张 host 图
├── platforms.py                平台适配器：Elf 与 MachO 两个类，库名、依赖表读取、二进制头里的架构、查找路径改写、系统库集合与 `ldconfig -p` 的架构标签、工具链；加平台即加一个类
...
资源路径是 `native/<引擎组>/<格式版本>/<平台>/`，`milvus` 表示两个引擎合成一组，`1` 是 `manifest.properties` 的 `format.version`。平台段在路径里，几个平台的 JAR 可以合进同一个 assembly 而不冲突，运行时 `NativeLibraries.platform()` 按当前操作系统和架构选目录；今天 sbt 只接受与构建机同平台的那一个包。storage-only 的 `native/<平台>/` 与独立 Knowhere 的 `native/knowhere/` 布局已从构建中删除（设计索引决策日志 2026-09-24）：`native-storage` 资源目录下残留的 `native/` 使构建失败，Dockerfile 拒绝 assembly 里出现 `native/knowhere/` 或 `native/linux-*/` 条目。
