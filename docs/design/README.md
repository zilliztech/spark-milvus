...
| 2026-09-11 | 我们这边的 v1 类名换成 v2 | backfill、ListV2SegmentsApp 与对应测试断言里的 WebIdentityTokenCredentialsProvider 与 EnvironmentVariableCredentialsProvider 换成 software.amazon.awssdk 的对应类；仓库里已无 v1 类名。但 aws-java-sdk-core 依赖不能删：云平台自己注入 spark.hadoop.fs.s3a.assumed.role.credentials.provider = com.amazonaws.auth.DefaultAWSCredentialsProviderChain，是 v1 类名且不在 hadoop-aws 的五条映射表里，实现类只来自我们 fat jar。要平台先改常量。这条记在 [storage-auth.html](architecture/storage-auth.html#platform-blocker) |
