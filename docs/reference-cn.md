...
| `milvus.read.batch.max.bytes` | Long | 否 | 33554432 | 每个原生 record batch 的正目标字节上限，映射为 `reader.record_batch_max_size`；当前上游最大值为 4294967296（4 GiB）。exact 搜索里它还是一次引擎调用拿到的 base 块：不设时 32 MiB；批量距离入口在内部分块，块只决定每次调用的固定开销和任务内存，32 MiB 与 128 MiB 实测每核速率相同。取值写进 INFO 日志 `exact scan batch: …`（设计见 [search-resources.html](design/architecture/search-resources.html#block)）。 |
