...
| `milvus.read.batch.max.bytes` | Long | 否 | 读 33554432；exact 搜索自动 | 每个原生 record batch 的正目标字节上限，映射为 `reader.record_batch_max_size`；当前上游最大值为 4294967296（4 GiB）。exact 搜索里它还是一次引擎调用扫的 base 块：不设时每个 executor 按本机 L3 × CPU 配额 × 0.6 ÷ 任务槽位算出，钳在 1 到 32 MiB 并取 2 的幂，读不到 L3 取 4 MiB，集群上 CPU 配额最多取 executor 核数与宿主机核数之比，没有 CPU limit 的 pod 不会把整机算作自己的，取值写进 INFO 日志 `exact scan batch: …`；显式设置则按设置值（设计见 [search-resources.html](design/architecture/search-resources.html#block)）。 |
