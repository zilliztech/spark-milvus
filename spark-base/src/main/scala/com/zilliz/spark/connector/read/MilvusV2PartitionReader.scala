...
class MilvusV2PartitionReader(
  ...
  // The derivation lives in ColumnBinding, so the columnar reader gets the
  // same answers rather than a second copy of the reasoning.
  private val task = setup.task
  private val columnGroups: Seq[V2ColumnGroup] = task.layout match {
    case SegmentLayout.ColumnGroups(groups) => groups
    case other =>
      throw new IllegalArgumentException(
        s"V2 reader needs a materialized column group layout, got $other"
      )
  }
  ...
  private def verifyRowCount(): Unit = {
    ...
    if (observedPhysicalRows != expectedTotalRows) {
      throw new IllegalStateException(
        s"V2 reader delivered $observedPhysicalRows rows for segment " +
