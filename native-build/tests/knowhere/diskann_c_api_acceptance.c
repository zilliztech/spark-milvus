...
/* This acceptance test requires a real Knowhere build with DiskANN enabled.
 * It owns the exact-distance configuration used by the connector's native
 * bundle without modifying the pinned Knowhere source checkout.
 */
#define ROWS 1000
...
main(void) {
    ...
    written = snprintf(build_parameters, sizeof(build_parameters),
                       ...
                       "\"refine_quant_type\":\"NONE\",\"shuffle_build\":false}",
