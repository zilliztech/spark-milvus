...
/** Tests for [[ParquetFooterReader]]. S3-backed reads need minio + hadoop-aws
  ...
  * arrow-cpp).
  */
class ParquetFooterReaderTest extends AnyFunSuite with Matchers {
  ...
  test("parseGroupFieldIdList handles the multi-field sample") {
    val parsed =
      ParquetFooterReader.parseGroupFieldIdList("100,0,1;101;102")
    parsed shouldBe Seq(Seq(100L, 0L, 1L), Seq(101L), Seq(102L))
  }

  test("parseGroupFieldIdList tolerates trailing semicolons") {
    ParquetFooterReader.parseGroupFieldIdList(
      "103;"
    ) shouldBe Seq(Seq(103L))
  }

  test("parseGroupFieldIdList on null/empty returns empty") {
    ParquetFooterReader.parseGroupFieldIdList(null) shouldBe Seq.empty
    ParquetFooterReader.parseGroupFieldIdList("") shouldBe Seq.empty
  }

  test("parseGroupFieldIdList on single group of one field") {
    ParquetFooterReader.parseGroupFieldIdList("103") shouldBe Seq(
      Seq(103L)
    )
  }
  ...
  test("readFieldIdsFromSchema returns per-column field ids in schema order") {
    ...
    try {
      val result = ParquetFooterReader.readFieldIdsFromSchema(
        tmp.toUri.toString,
        localStore
      )
  ...
  test("readFieldIdsFromSchema does not swallow fatal errors") {
    ...
    intercept[OutOfMemoryError] {
      ParquetFooterReader.readFieldIdsFromSchema("file.parquet", store)
    }
  }
