...
import org.apache.spark.sql.types.{ArrayType, FloatType, LongType, StringType, StructType}
...
object F16VectorInsertDemo {
  private val log = LoggerFactory.getLogger(getClass)

  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder().master("local[*]")
      .appName("F16VectorDemo")
      .getOrCreate()
    ...
    val uri = ""
    val token = ""
    val collectionName = "f16_vector_test"
    ...
    val vectorField: String = "float_vector_field"
    ...
    val df = spark.read
      .schema(new StructType()
        ...
        .add(vectorField, ArrayType(FloatType))
      )
      .json(filePath)
